# -*- coding: utf-8 -*-
"""小红书关键词搜索链接采集 — thin wrapper.

业务逻辑在 services.keyword_search.xiaohongshu，本文件只保留 standalone CLI 入口。
"""
...
async def run_xiaohongshu_link_search(task, ctx) -> dict[str, Any] | None:
    """供 task_runner 调用的入口。"""
    from services.keyword_search import get_keyword_search_service

    return await get_keyword_search_service("xiaohongshu").run(task, ctx)


# ===== Standalone CLI =====

_DEFAULT_KEYWORDS = ["SKG"]
_DEFAULT_CONTEXT_KEY = "xiaohongshu-context:default:placeholder"


async def _run_standalone() -> None:
    parser = argparse.ArgumentParser(description="Standalone Xiaohongshu product link search")
    parser.add_argument("keywords", nargs="*", default=_DEFAULT_KEYWORDS, help="搜索关键词")
    parser.add_argument("--context-key", default=os.getenv("XHS_CONTEXT_KEY", _DEFAULT_CONTEXT_KEY))
    parser.add_argument("--limit", type=int, default=1000)
    parser.add_argument("--max-pages", type=int, default=5)
    parser.add_argument("--output", default="/tmp/xhs_links.json")
    ...
    svc = get_keyword_search_service("xiaohongshu")
    ...
    session_result = await agent_bay.create(
        CreateSessionParams(
            labels={"app": "micro-sniper", "kind": "standalone", "script": "xiaohongshu_link_search"},

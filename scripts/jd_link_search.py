# -*- coding: utf-8 -*-
"""京东关键词搜索链接采集 — thin wrapper。

业务逻辑在 services.keyword_search.jd，本文件只保留 standalone CLI 入口。
"""
...
async def run_jd_link_search(task, ctx) -> dict[str, Any] | None:
    """供 task_runner 调用的入口。"""
    from services.keyword_search import get_keyword_search_service

    return await get_keyword_search_service("jd").run(task, ctx)
...
async def _run_standalone() -> None:
    ...
    svc = get_keyword_search_service("jd")

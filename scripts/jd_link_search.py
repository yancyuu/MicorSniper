...
async def run_jd_link_search(task: Task, ctx: BrowserContext) -> dict[str, Any] | None:
    ...
    try:
        ...
        login_result = await check_login_status(agent, "jd")
        if login_result.logged_in:
            await task.log_step(step_counter[0], "登录态校验通过（京东）", {"platform": "jd"}, {"logged_in": True}, "completed")
        else:
            await task.log_step(step_counter[0], "登录态校验失败（京东）", {"platform": "jd"}, {"logged_in": False, "reason": login_result.reason}, "failed")
            await task.fail(login_failed_message("jd"))
            return None
        step_counter[0] += 1
        ...
        try:
            ...
            logger.warning(f"[jd_link_search] Failed to close browser (continuing): {e!r}")

        try:
            await asyncio.wait_for(agent_bay.delete(session, sync_context=False), timeout=30)
        except Exception as e:
            logger.warning(f"[jd_link_search] Failed to delete session: {e!r}")
...
async def _collect_keyword_links(
    ...
    while page_index <= max_pages and len(collected) < remaining:
        ...
        if pages_per_batch > 0 and pages_in_batch >= pages_per_batch:
            logger.info(f"[jd_link_search] keyword={keyword} batch limit reached ({pages_per_batch} pages), pausing at page {page_index}")
            break

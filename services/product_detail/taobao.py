...
() => {
    ...
    // 标题：新版淘宝详情页 class 名不稳定，优先用 SSR 数据。
    result.title = cleanText(item.title);
    if (!result.title) {
        result.title = firstText(['h1', '[class*="ItemHeader--"] [class*="title"]', '[data-spm="1000983"]']).slice(0, 500);
    }
    if (!result.title) {
        const titleMatch = bodyText.match(/图文详情\\s+(.{8,200}?)\\s+(已售|券后|优惠促销|价格)/);
    ...
    if (!result.shop_name) {
        const shopEl = document.querySelector('[class*="shopName"], [class*="ShopName--"], [class*="shopNameText"], [class*="ShopHeader"] a[title]');
    ...
    result.location = locEl ? cleanText(locEl.textContent) : '';
    if (!result.location) {
...
class TaobaoProductDetailService(ProductDetailService):
    platform = "taobao"
    detail_js = TAOBAO_DETAIL_JS
